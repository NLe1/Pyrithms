    ...
    def test_union_find(self, num_objects: int = 10, num_ops: int = 100):
        ...
        for _ in range(10):
          ...
          for _ in range(num_ops):
            index1, index2 = random.randint(start_index, end_index), random.randint(start_index, end_index)
            while index1 == index2:
              index1, index2 = random.randint(start_index, end_index), random.randint(start_index, end_index)
            new_union.union(index1, index2)
            # test finding
            self.assertEqual(find_root(new_union, index1), find_root(new_union, index2))
